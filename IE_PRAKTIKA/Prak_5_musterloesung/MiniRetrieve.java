...
public class MiniRetrieve {
	...
	private void processQueries(){
		...
		while (itQueryIds.hasNext()) {
			...
			while (itTerms.hasNext()) { 
				...
				if(!idf.containsKey(currentQueryTerm)) idf.put(currentQueryTerm, Math.log(1+ numberOfFiles));
				...
				if(myInvertedIndex.containsKey(currentQueryTerm)){
					...
					while (itmap.hasNext()) {
						...
						else accuHash.put(currentFileName,a * b);				  
				    } // end while
				}
			}//end while		
	...
	private void createIndexes(String directory){
		...
		if(myDirectory.isDirectory() && myDirectory.exists()){
			File[] files =myDirectory.listFiles(); //File-Array mit allen Dateien im Verzeichnis
			...
			for (int i=0;i<files.length;i++) { //allen Files durchlaufen und Inhalt indexieren
		        ...
		        for (int j=0; j<tokens.length; j++){ 			
					...
					if(myInvertedIndex.containsKey(currentToken)){ //Existiert Token schon
						...
						else myInvertedIndex.put(filename,currentToken,1); //Falls Filename noch nicht existiert (Token aber schon) setze Anzahl auf 1
					}
					else{
		...
		else System.out.println("Verzeichnis nicht gefunden. Bitte Verzeichnis korrekt angeben!");
	}
